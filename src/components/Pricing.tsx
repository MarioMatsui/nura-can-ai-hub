...
const plans = {
  ...
  medico: {
    ...
    monthlyPrice: 14.9,
    monthlyOriginalPrice: 39.9,
    annualPrice: 119.9,
    annualOriginalPrice: 27.9,
    annualTotalPrice: 119.9,
    popular: false,
  },
  juridico: {
    name: "Jurídico",
    ...
    return (
      <div className="flex flex-col items-center">
        <div className="relative inline-flex items-center justify-center">
          <span className="text-2xl sm:text-3xl font-bold">R$ {currentPrice.toFixed(2).replace(".", ",")}</span>
          {showOriginal && (
            <span className="absolute -top-2 -right-2 text-xs font-semibold text-muted-foreground dark:text-white/50 line-through decoration-red-500 decoration-2">
                    ...
                    <div key={key} className="flex flex-col h-full">
                      ...
                      <div
                        className={`text-[11px] text-center mt-2 ${
                          isAnnual && plan.monthlyPrice > 0 ? "text-muted-foreground" : "invisible"
                        }`}
